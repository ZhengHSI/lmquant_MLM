...
__all__ = ["LlmModelStruct", "LlmDecoderLayerStruct", "LlmBackboneStruct"
          ,"VitModelStruct", "VitDecoderLayerStruct", "VitBackboneStruct"]
...
class LlmBackboneStruct:
    ...
    def layer_structs(self) -> list["LlmDecoderLayerStruct"]:
        ...
        # # 只提取最后 8 层
        #     self._layer_structs = [
        #         extract_llm_layer(layer, layer_idx, self) 
        #         for layer_idx, layer in enumerate(self.layers[-8:])
        #     ]
...
class LlmDecoderLayerStruct:
    ...
    def update_full_name(self, new_full_name: str):
        """Update the full_name and all related attributes."""
        self.full_name = new_full_name
        # 更新所有依赖 full_name 的属性
        self.attn_ln_full_name = f"{self.full_name}.{self.attn_ln_name}"
        self.attn_block_full_name = (
            f"{self.full_name}.{self.attn_block_name}" if self.attn_block_name else self.full_name
        )
        self.ffn_ln_full_name = f"{self.full_name}.{self.ffn_ln_name}"
        self.ffn_block_full_name = (
            f"{self.full_name}.{self.ffn_block_name}" if self.ffn_block_name else self.full_name
        )
        self.proj_qkv_full_names = [f"{self.attn_block_full_name}.{name}" for name in self.proj_qkv_names]
        self.proj_out_full_name = f"{self.attn_block_full_name}.{self.proj_out_name}"
        ...
        if self.config.num_experts > 1:
            ...
            self.experts_full_name = self.ffn_block_full_name
            self.expert_full_names = [self.experts_full_name]
            self.router_full_name = ""
...
def extract_llm(model: nn.Module) -> LlmModelStruct | None:
    ...
    if isinstance(model, (OPTForCausalLM, OPTForSequenceClassification, OPTForQuestionAnswering)):
        ...
        backbone_name = "model"
    elif "MiniCPMV" in str(model.__class__):
        backbone = model.llm.model
        backbone_name = "llm.model"
    else:
...
def extract_llm_backbone(backbone: nn.Module, parent: LlmModelStruct) -> LlmBackboneStruct | None:
    ...
    elif "MiniCPMModel" in str(backbone.__class__):
        embeddings = [backbone.embed_tokens]
...
def extract_llm_layer(layer: nn.Module, layer_idx: int, parent: LlmBackboneStruct) -> LlmDecoderLayerStruct | None:
    ...
    if isinstance(layer, OPTDecoderLayer):
        ...
        experts_name = ""
        router_name = ""
    elif "MiniCPMDecoderLayer" in str(layer.__class__):
        attn_ln = layer.input_layernorm
        attn_block = layer.self_attn
        assert "MiniCPMSdpaAttention" in str(attn_block.__class__)
        ffn_ln = layer.post_attention_layernorm
        ffn_block = layer.mlp
        assert "MiniCPMMLP" in str(ffn_block.__class__)
        proj_qkv = [attn_block.q_proj, attn_block.k_proj, attn_block.v_proj]
        proj_out = attn_block.o_proj
        proj_1st = [ffn_block.up_proj, ffn_block.gate_proj]
        proj_2nd = [ffn_block.down_proj]
        ...
        if "MiniCPMDecoderLayer" in str(layer.__class__):
            attn_block_kwargs = attn_block_kwargs[:-1]
        attn_ln_name = "input_layernorm"
        attn_block_name = "self_attn"
        proj_qkv_names = ["q_proj", "k_proj", "v_proj"]
...
class VitModelStruct:
    ...
    fc: nn.Linear | None
    backbone_name: str
    config: VitConfigStruct
    fc_name: str

    _backbone_struct: tp.Optional["VitModelStruct"] = None
    ...
    @property
    def backbone_struct(self) -> "VitBackboneStruct":
        """Extract backbone."""
        if self._backbone_struct is None:
            self._backbone_struct = extract_vit_backbone(self.backbone, self)
        return self._backbone_struct

    @staticmethod
    def build(model: nn.Module) -> tp.Optional["VitModelStruct"]:
        """Build the Large Language Model Structure."""
        return extract_vit(model)

@dataclass
class VitBackboneStruct:
    """Large Language Model Backbone Structure."""

    module: nn.Module
    """the nn.Module of the backbone"""
    parent: VitModelStruct
    # region modules inside backbone
    embeddings: list[nn.Embedding]
    """list of embeddings [embed_tokens, embed_positions]"""
    proj_in: nn.Linear | None
    ...
    # endregion
    # endregion
    _layer_structs: list["VitDecoderLayerStruct"] | None = None
    ...
    @property
    def config(self) -> VitConfigStruct:
        """Get the config."""
        return self.parent.config
    ...
    def layer_structs(self) -> list["LlmDecoderLayerStruct"]:
        ...
        if self._layer_structs is None:
            self._layer_structs = [
                extract_vit_layer(layer, layer_idx, self) for layer_idx, layer in enumerate(self.layers)
            ]
        return self._layer_structs

@dataclass
class VitDecoderLayerStruct:
    """Large Language Model Decoder Layer."""

    module: nn.Module
    """the nn.Module of the block."""
    parent: VitBackboneStruct
    idx: int
    # region modules inside decoder layer
    attn_ln: nn.LayerNorm
    attn_block: nn.Module
    ...
    def __post_init__(self):
        assert len(self.proj_qkv) == 3
        assert len(self.proj_2nd) == self.config.num_experts
        assert len(self.proj_1st) == self.config.num_experts
...
def extract_vit(model: nn.Module) -> VitModelStruct | None:
    """Extract llm into components."""
    if "MiniCPMV" in str(model.__class__):
        backbone = model.vpm
        backbone_name = "vpm"
    else:
        raise ValueError(f"Unsupported model type: {type(model)}")
...
def extract_vit_backbone(backbone: nn.Module, parent: VitModelStruct) -> VitBackboneStruct | None:
    """Extract llm backbone into components."""
    if "SiglipVisionTransformer" in str(backbone.__class__):
...
def extract_vit_layer(layer: nn.Module, layer_idx: int, parent: VitBackboneStruct) -> VitDecoderLayerStruct | None:
    """Extract llm block.
    ...
    Returns:
        LlmBlockStruct: Block.
    """
    if "SiglipEncoderLayer" in str(layer.__class__):
        attn_ln = layer.layer_norm1
        attn_block = layer.self_attn
        assert "SiglipAttention" in str(attn_block.__class__)
        ffn_ln = layer.layer_norm2
        ffn_block = layer.mlp
        assert "SiglipMLP" in str(ffn_block.__class__)
        proj_qkv = [attn_block.q_proj, attn_block.k_proj, attn_block.v_proj]
        proj_out = attn_block.out_proj
